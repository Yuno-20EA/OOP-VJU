...
public:
    ...
    void saveToFile(string filename) {
        ...
        ofstream f(filename);
        if (!f.is_open()) {
            cerr << "Loi.File dang trong hoac dinh dang khong hop le !! " << filename << endl;
            return;
        }
        for (const auto& cb : dsCanBo) {
            json j_items;
            cb->to_json(j_items);
            j_array.push_back(j_items);
        }
        f<< j_array.dump(4);
        f.close();
        cout << "Da luu file thanh cong !!" << filename << endl;
    }
    void loadToFile(string filename){
        ifstream f(filename);
        if (!f.is_open()) {
            cerr << "Loi.File khong ton tai hoac dang bi khoa!!" << endl;
            return;
        }
        // giai ma json
        json j_array;
        try {
            ...
            cerr << "LOI:File khong hop le !!" << endl;
            return;
        }
        f.close();
        // xay dung bang lookup table (anh xa tu string sang ham khoi tao)
        map <string, function<unique_ptr<CanBo>(const json&)>> factory;
        factory["CongNhan"] = [] (const json& j_canbo) {
            return make_unique<CongNhan>(j_canbo.at("Name").get<string>(), j_canbo.at("Age").get<int>(),
                                         j_canbo.at("Gender").get<string>(), j_canbo.at("Address").get<string>(),
                                         j_canbo.at("Level").get<int>());
        };
        factory["KySu"] = [] (const json& j_canbo) {
            return make_unique<KySu>(j_canbo.at("Name").get<string>(), j_canbo.at("Age").get<int>(),
                                     j_canbo.at("Gender").get<string>(), j_canbo.at("Address").get<string>(),
                                     j_canbo.at("FieldName").get<string>());
        };
        factory["NhanVien"] = [] (const json& j_canbo) {
            return make_unique<NhanVien>(j_canbo.at("Name").get<string>(), j_canbo.at("Age").get<int>(),
                                         j_canbo.at("Gender").get<string>(), j_canbo.at("Address").get<string>(),
                                         j_canbo.at("Job").get<string>());
        };
        // duyet va nap cac du lieu moi
        dsCanBo.clear();
        for (const auto& items : j_array) {
            string type = items.at("Type").get<string>();
            if (factory.count(type)) {
                dsCanBo.push_back(factory[type](items));
            }
        }
        cout << "Da them thanh cong" << dsCanBo.size() << " can bo tu file." << endl;
    };
    void TimCanBo() {
        string keyword;
    ...
    void runcase() {
        ...
        while (true) {
            ...
            cout << "1. Them moi | 2. Tim kiem | 3. Hien thi | 4.Luu File | 5.Nap File | 6.Thoat \nLua chon: ";
            if (!(cin>>chon)) {
            ...
            switch (chon) {
                ...
                case 3:
                    display(cout);
                    break;
                case 4:saveToFile("QLCB.json");
                    break;
                case 5:loadToFile("QLCB.json");
                    break;
                default:
